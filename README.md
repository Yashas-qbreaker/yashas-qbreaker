<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?size=28&duration=3000&color=00F7FF&center=true&vCenter=true&width=700&lines=Hi+I'm+Yashas;AI+Developer;Blockchain+Enthusiast;Cybersecurity+Learner;Building+Scalable+Technology+Solutions" />
</p>

# Hi 👋 I'm Yashas

🚀 Technology enthusiast working on **AI, Blockchain, and Cybersecurity**

## 🧠 About Me

* 🤖 Built an **AI Voice Assistant**
* 🌍 Developing **BeaconAid – Disaster Management Platform**
* 🔗 Created a **Blockchain Voting System**
* 🧠 Interested in **Systems Programming & AI**
* 🐧 Exploring **Linux Internals**
* 🔐 Learning **Cybersecurity**

## 🛠 Tech Stack

<p>
<img src="https://skillicons.dev/icons?i=python,js,ts,react,nodejs,linux,docker,git" />
</p>

* Python
* React.js / TypeScript
* AI / Machine Learning
* Blockchain
* Linux
* Cloud & Web Development

## 📌 Featured Projects

* **BeaconAid** – Disaster response and community coordination platform
* **AI Voice Assistant (Syrix)** – Hands-free AI assistant with wake word detection
* **Voting Platform** – Blockchain-based secure voting system
* **EventsHub** – Event discovery and management system

## 🌐 Communities

* Google Student Ambassador
* GDG Core Member
* IEEE Volunteer

## 📊 GitHub Stats

<p align="center">
  <img src="https://streak-stats.demolab.com?user=Yashas-qbreaker&theme=tokyonight" height="170"/>
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=Yashas-qbreaker&layout=compact&theme=tokyonight" height="170"/>
</p>

## 📈 3D Contribution Graph

![](https://raw.githubusercontent.com/Yashas-qbreaker/Yashas-qbreaker/gh-pages/profile-night-green.svg)

⭐ Always exploring new technologies and building impactful solutions
